...
| 25.141 | **G.63 — Lambda resource-based policy + ESM writes + alias edit** — backend `lambda.ts` +6 routes (GET/POST/DELETE policy with ResourceNotFound→null arm + parsed Statement, POST event-source-mappings with TRIM_HORIZON default + conditional ScalingConfig, PUT event-source-mappings/:uuid, PUT aliases/:aliasName with RoutingConfig from routingAdditionalVersion), `useLambda` +6 hooks (useLambdaPolicy gated query, useAddLambdaPermission, useRemoveLambdaPermission, useCreateEventSourceMapping, useUpdateEventSourceMapping, useUpdateLambdaAlias), LambdaDashboard Permissions tab (principal Service/AWS/stringify/dash mapping arms, Add-permission modal disabled-until-valid, per-statement DeleteButton, dual error alerts) + Triggers create/edit modals (ARN gate, position Select, NaN-guarded batch/concurrency, enabled checkbox) + Aliases Edit modal (prefill, empty-save undefined arms). Tests: lambda.test.ts 67→90, useLambda.test.ts 47→54, LambdaDashboard.test.tsx 66→93 — dialog-scoped within(dialogOf()) interactions for stay-mounted modals, regex dialogOf for split header text nodes. Verification: all 3 files 100% on all metrics, typecheck clean, make test-cov exit 0 | Done | 2026-08-22 |
| 25.142 | **G.73 — SQS native message move tasks (DLQ redrive)** — backend `sqs.ts` +3 routes (start/list/cancel; list maps TaskHandle/SourceArn/DestinationArn-null/MaxRate/Status/Moved/ToMove/StartedTimestamp/FailureReason-null from Floci's Results array), `useSQS` +3 hooks, SQSPage DLQTab rebuilt: sources container + Message move tasks container (Start redrive modal with destination URL -> ARN via exported `queueUrlToArn` (region parsed from sqs.{region} hostnames, account/name from path parts, invalid-URL catch fallback), max-rate NaN-guarded, disabled-until-destination; task table with 24-char truncated handles, COMPLETED/FAILED/RUNNING status indicators, moved/toMove ?? 0 dashes, started-date formatting, failureReason dash, row-Cancel suppressed for terminal statuses, success/error toasts). Tests: sqs.test.ts 54→63 (+9: every route happy/400/sparse/maxResults-passthrough/command assertions), useSQS.test.ts 24→28 (+4), SQSPage.test.tsx 67→81 (+14 incl. direct queueUrlToArn unit tests for regional-host/root-path/invalid-URL arms; dialog-scoped Cancel filtering since hidden modals keep footer Cancels mounted). Verification: all 3 files 100% on all metrics, typecheck clean, make test-cov exit 0 (272 files, 100% repo-wide) | Done | 2026-08-22 |
| 25.143 | **G.29 closeout — RDS tags CRUD** — backend `rds.ts` +3 routes (GET /tags?arn=, POST /tags {arn,tags}, DELETE /tags {arn,tagKeys} with 400 guards on arn/tags/tagKeys); `useRDS` +3 hooks (useRDSTags gated on arn, useRDSAddTags, useRDSRemoveTags with cache invalidation); RDSDashboard shared RDSTagsEditor component rendered in DB instance + DB cluster detail (tags table with per-tag DeleteButton + loading match on removeTags.variables, add-tag key/value inputs disabled-until-key, dual dismissible error alerts with message fallbacks). Tests: rds.test.ts 59→70 (+11), useRDS.test.ts 31→34 (+3), RDSDashboard.test.tsx 118→129 (+11 incl. cluster-detail editor render, pending-delete disabled gate, alert dismiss firing resets); ServicePage.test.tsx useRDS mock extended with tags stubs. Verification: all 3 files 100% on all metrics, typecheck clean, make test-cov exit 0 (272 files, 9,751 tests, 100% repo-wide) | Done | 2026-08-22 |
| 25.144 | **G.60 — CodeBuild report groups + retry + project edit** — backend `codebuild.ts` +7 routes (PUT /projects/:name UpdateProject with description/source/artifacts/environment/serviceRole/timeouts/encryptionKey/tags, POST /builds/:id/retry, GET/POST /report-groups + GET/PUT/DELETE /report-groups/:arn with shared mapReportGroup sparse fallbacks and 400 guards on create name/type); `useCodeBuild` +7 hooks (retry, update project, report-group list/create/delete/update); CodeBuildDashboard: project Edit modal (dash→empty prefill), builds Retry action with in-flight variables match, Report Groups container (type/export dash fallbacks, create modal with Test/Series Select + optional S3 bucket/path, per-row delete with pending-match). Tests: codebuild.test.ts 27→39, useCodeBuild.test.ts 17→23, CodeBuildDashboard.test.tsx 49→66 (dialog-scoped within() queries for stay-mounted Cloudscape modals, hidden-class close assertions, select option flows). Verification: all 3 files 100% on all metrics, typecheck clean, make test-cov exit 0 (272 files, 9,792 tests, 100% repo-wide) | Done | 2026-08-22 |
| 25.145 | **G.36 — CodeDeploy on-premises instances + deployment lifecycle ops** — backend `codedeploy.ts` +11 routes (GET /on-prem-instances List+BatchGet with registrationStatus filter + sparse fallbacks, POST /on-prem-instances Register with optional IAM fields + 400, GET /on-prem-instances/:name, DELETE /on-prem-instances/:name Deregister, POST /on-prem-instances/tags + /untag with 400 guards on names/tags, POST /deployments/:id/continue, POST /deployments/:id/lifecycle-hook-status with 400 on missing executionId, GET + POST /deployments/:id/targets List/BatchGet with 400 on empty targetIds); `useCodeDeploy` +9 hooks (on-prem list/register/deregister/add+remove tags, continue, put hook status, targets list-then-batchget composite query gated on id); CodeDeployDashboard On-Premises tab (register modal, tag add/edit modal with Remove-tag, per-row deregister with pending-match) + deployment row actions (Continue with in-flight match, Targets modal with targets table + hook execution-id form + Succeeded/Failed/Pending select). Dead code removed: `(i.tags \|\| [])` in tags column (mapper already normalizes). Tests: codedeploy.test.ts 47→67, useCodeDeploy.test.ts 17→27, CodeDeployDashboard.test.tsx 48→78 (tab navigation, dialog-scoped within() queries, Escape dismiss helper, load-arm via className match, close asserts via button disappearance since aria-hidden hides mounted modals); ServicePage useCodeDeploy mock extended. Verification: all 3 files 100% on all metrics, typecheck clean, make test-cov exit 0 (272 files, 9,856 tests, 100% repo-wide) | Done | 2026-08-22 |
| 25.146 | **G.28 closeout — Config Service tags CRUD** — backend `configservice.ts` +3 routes (GET /tags?arn= maps Tags Key/Value, POST /tags {arn,tags}, POST /tags/untag {arn,tagKeys} with 400 guards on arn/tags/tagKeys); `useConfigService` +3 hooks (useConfigTags gated on arn, useAddConfigTags, useRemoveConfigTags with invalidation); ConfigServiceDashboard shared ConfigTagsModal (tags table with per-tag DeleteButton + pending-match on removeTags.variables, add-tag form disabled-until-key, dual dismissible error alerts with fallbacks) opened via Tags action on config rule rows (disabled when ConfigRuleArn absent) and conformance pack rows (disabled when ARN dash). Tests: configservice.test.ts 29→40 (+11), useConfigService.test.ts 13→16 (+3), ConfigServiceDashboard.test.tsx 54→68 (+14 incl. tab navigation for pack rows, missing-ARN disabled gates, alert dismiss firing resets, pending-delete disabled via className). Verification: all 3 files 100% on all metrics, typecheck clean, make test-cov exit 0 (272 files, 9,885 tests, 100% repo-wide) | Done | 2026-08-22 |
| 25.147 | **G.65 closeout — EventBridge pattern tester + bus tags** — backend `events.ts` +4 routes (POST /test-event-pattern TestEventPattern with 400 on missing pattern/event, GET /tags?arn= maps Tags via ResourceARN, POST /tags TagResource, DELETE /tags UntagResource with 400 guards — activating three previously dead command imports); `useEvents` +4 hooks (useTestEventPattern, useEventTags gated on arn, useAddEventTags, useRemoveEventTags); EventsPage new "Pattern Tester" tab (pattern + event textareas, Test button with match/no-match alerts + error toasts with fallbacks) and EventTagsModal opened via Tags action on event-bus rows (disabled when Arn absent) with per-tag delete + add-tag form + success/error toasts. Tests: events.test.ts 63→78 (+15 incl. Result-false mapping + DELETE-with-body raw router.request helper), useEvents.test.ts 41→45 (+4), EventsPage.test.tsx 102→118 (+16 incl. match/no-match/error/fallback tester flows, tags modal CRUD, no-ARN disabled gate, fireEvent.change for textarea onChange since multiple Cloudscape tabs stay mounted). Verification: all 3 files 100% on all metrics, typecheck clean, make test-cov exit 0 (272 files, 9,921 tests, 100% repo-wide) | Done | 2026-08-22 |
| 25.148 | **Batch sweep A — seven quick gaps closed** — G.61 CodeDeploy Stop+UpdateDeploymentGroup (+2 routes, Stop row action); G.72 ECR PutImageTagMutability (+1 route, Mutability modal); G.80 CE GetCostAndUsageWithResources (+1 route, by-resource button); G.74 Firehose Tag/UntagDeliveryStream (+2 routes, Tags modal); G.68 Transcribe vocabulary CRUD (+3 routes, table+modal); G.59 Secrets resource policy/batch-value/version-stage (+5 routes, Resource policy tab); G.58 KMS policy/sign/verify/MAC/rotate-on-demand (+7 routes, Sign/Verify + Key policy tabs). Stale/N-A marks: G.30 (no Floci s3control), G.48 (no Floci ThingGroups), G.75 (Floci rejects BatchExecuteStatement). ServicePage mocks extended for all new hooks. Verification: make test-cov exit 0 (272 files, 10,039 tests, 100% all metrics repo-wide) | Done | 2026-08-22 |
| 25.149 | **Batch sweep B — six more gaps closed** — G.53 SES templates (7 routes: CRUD + TestRenderTemplate + SendTemplatedEmail; Email Templates section + Preview + two modals); G.49 ECS capacity providers (5 routes incl. PutClusterCapacityProviders; Capacity Providers tab with create modal); G.46 SSM Run Command (5 routes; Run Command tab with history/invocations panel/cancel/send modal); G.57 IAM (4 routes: role inline policies PUT/DELETE, trust-policy PUT, POST /simulate; role detail modal Update-trust-policy + Inline-policies containers); G.55 Backup recovery points (3 routes; expandable per-vault table); G.45/G.92 CloudMap register/deregister/health/discover (4 routes; Register modal + per-row deregister). Verification: make test-cov exit 0 (272 files, 10,206 tests, 100% all metrics repo-wide) | Done | 2026-08-22 |
| 25.150 | **Batch sweep C — six more gaps closed** — G.69 Glue (UpdateTable/UpdateDatabase/GetTableVersions/BatchDeleteTable + edit modal + versions panel); G.70 Neptune/ElastiCache (ModifyDBCluster PATCH + ModifyDBInstance; ModifyReplicationGroup + ModifyUser POST; Neptune Modify modal); G.78 STS federation (AssumeRoleWithSAML/WebIdentity, GetFederationToken, DecodeAuthorizationMessage + Federation tab); G.76 ACM (ImportCertificate/ExportCertificate/Add+RemoveTags + Import/Export modals); G.67 Transfer (UpdateServer/UpdateUser/Import+DeleteSshPublicKey + Edit modal + SSH import form); G.43 EMR (steps + instance-groups detail panel, backend pre-existing, new useEMRInstanceGroups hook). ServicePage ACM mock extended. Verification: make test-cov exit 0 (272 files, 10,324 tests, 100% all metrics repo-wide) | Done | 2026-08-22 |
| 25.151 | **Batch sweep D (partial) — EC2 trio + OpenSearch** — G.50/31/79 EC2 (7 routes: prefix-lists, spot request/list/cancel with 400 guards, SG rules list with group filter, ModifySecurityGroupRules, UpdateSecurityGroupRuleDescriptionsIngress; Spot Requests + Prefix Lists + SG Rules tabs with request modal); G.56 OpenSearch (UpdateDomainConfig, UpgradeDomain with check-only, List/AddTags via ?arn=; Manage modal). ServicePage OpenSearch mock extended (MSK block repaired after edit collision). Verification: make test-cov exit 0 (272 files, 10,385 tests, 100% all metrics repo-wide) | Done | 2026-08-22 |
| 25.152 | **Batch sweep E (final) — AppSync + Cognito closes, stale/N-A marks** — G.64 AppSync (resolver create/update/delete + UpdateDataSource routes; create modal + row delete on Resolvers tab); G.77 Cognito (AdminAddUserToGroup route + hook + Add-to-group modal; rest pre-existing); G.88 SNS N/A (no Floci support); #2 Cognito priority closed; G.42 stale→Done; G.54/G.66 N/A (Floci handler limits). Verification: make test-cov exit 0 (272 files, 10,409 tests, 100% all metrics repo-wide) | Done | 2026-08-22 |
| 25.153 | **G.92 completion — CloudMap operations, instance detail, DNS namespaces** — backend `cloudmap.ts` +5 routes (POST /namespaces/private-dns + /public-dns with name/vpc 400 guards and sparse Namespace fallbacks; GET /operations ListOperations id/status map; GET /operations/:id GetOperation with create/update/targets mapping; GET /services/:id/instances/:instanceId GetInstance with attributes `|| {}`); `useCloudMap` +5 hooks; CloudMapDashboard: namespace create modal with HTTP/Private DNS/Public DNS Select (VPC field conditional, disabled-until-valid, error fallback across all three create mutations), Operations table with click-to-expand detail panel (dates/targets dash fallbacks), instance rows gain Detail toggle + id-link with attribute map rendering. Tests: cloudmap 25->39, useCloudMap 11->18, CloudMapDashboard 28->43 — all files 100% on all metrics. Verification: make test-cov exit 0 (272 files, 10,443 tests, 100% repo-wide) | Done | 2026-08-22 |
| 25.154 | **M.5 — Kinesis Analytics V2 dashboard** — new `kinesisanalytics.ts` backend (15 ops via 12 routes: applications list/detail/create/update/delete with CreateTimestamp guard, start/stop, snapshots list/create/detail/delete, tags list/add/remove via ?arn=) + `useKinesisAnalytics` (13 hooks) + `KinesisAnalyticsDashboard` (Applications tab with status indicators + start/stop/delete, Snapshots tab gated on selection, Detail panel, create-app + create-snapshot modals; @v8 ignore pattern per AmazonMQ precedent). Registered as `kinesisanalytics` under Analytics. SDK `@aws-sdk/client-kinesis-analytics-v2` added. Tests: 29 backend + 13 hook + 8 component — all at 100%. Gate: make test-cov exit 0 (287 files, 11,063 tests, 100% repo-wide) | Done | 2026-08-26 |
| 25.155 | **M.6 — EMR Serverless dashboard** — new `emrserverless.ts` backend (7 routes: applications list/detail/create (name/releaseLabel/type 400 guards)/update (auto-start/stop toggles)/delete/start/stop) + `useEMRServerless` (7 hooks) + `EmrServerlessDashboard` (status indicators, state-gated Start/Stop, detail panel with auto-config toggles, create modal). Registered `emrserverless` under Analytics. SDK added. Also fixed latent 3-dot mock path bug in AmazonMQ/Lightsail/EMR-Serverless/KinesisAnalytics tests (`../../../hooks` → `../../hooks` — mocks silently never applied, real hooks fetched, causing teardown AbortError flake in full runs). Tests: 10 backend + 6 hook + 8 component — 100%. Gate: make test-cov exit 0 (290 files, 11,085 tests, 100% repo-wide) | Done | 2026-08-26 |
| 25.156 | **M.10 — MWAA dashboard** — new `mwaa.ts` backend (7 routes: environments list, detail, create (name/sourceBucketArn/executionRoleArn 400 guards + DagS3Path/NetworkConfiguration defaults), update, delete, web-token, cli-token) + `useMWAA` (7 hooks) + `MWAADashboard` (env table with string-or-object names, web/CLI token actions, delete via real confirm dialog, create modal with 6 fields, detail panel). Registered `mwaa` under Analytics. SDK added. Tests: 15 backend + 6 hook + 10 component — 100%. Gate: make test-cov exit 0 (293 files, 11,116 tests, 100% repo-wide; one ec2-terminal parallel-load flake disappeared on rerun) | Done | 2026-08-27 |
| 25.157 | **M.11 — CloudWatch RUM dashboard** — new `rum.ts` backend (5 routes: appmonitors list (nextToken forwarded + surfaced), detail (optional-field defaults: domainList [] / created,lastModified "" / tags undefined), create (name/domain 400 guards), update, delete) + `useRUM` (5 hooks) + `RUMDashboard` (state indicators, delete clears open detail panel, create modal name/domain/platform). Registered `rum` under Analytics. SDK added. Tests: 11 backend + 5 hook + 6 component — 100%. Gate: make test-cov exit 0 (296 files, 11,138 tests, 100% repo-wide) | Done | 2026-08-27 |
| 25.158 | **M.13 — Bedrock AgentCore dashboard** — new `bedrockagentcore.ts` backend spanning both Floci controllers: control plane (runtimes list/detail/create (name/roleArn 400 guards, artifact+network defaults)/update/delete) + data plane (`POST /invoke/:arn` with payload guard, qualifier/contentType options, stream-to-text response). Uses @aws-sdk/client-bedrock-agentcore-control + bedrock-agentcore. Frontend: `useAgentCore` (6 hooks) + `AgentCoreDashboard` (runtimes table with status indicators, JSON invoke textarea in detail panel rendering raw response, create modal). Registered `bedrockagentcore` under AI/ML. Tests: 14 backend + 6 hook + 7 component — 100%. Gate: make test-cov exit 0 (299 files, 11,165 tests, 100% repo-wide) | Done | 2026-08-27 |
| 25.159 | **M.7 — FIS dashboard** — new `fis.ts` backend (9 routes: templates list/detail/create/update/delete with `nstr`/`statusOf` normalization helpers for SDK-vs-Floci shape drift (state as object), experiments list/detail/start (templateId guard)/stop-via-DELETE matching Floci+AWS mapping) + `useFIS` (8 hooks) + `FisDashboard` (two tabs: templates w/ start-experiment + delete actions, experiments w/ Stop gated to running; create modal w/ JSON actions field, invalid JSON rejected client-side; detail panel). Registered `fis` under Resilience. SDK added (@aws-sdk/client-fis, note class name is FisClient not FISClient). Tests: 22 backend + 8 hook + 8 component — 100%. Gate: make test-cov exit 0 (302 files, 11,203 tests, 100% repo-wide) | Done | 2026-08-27 |
| 25.160 | **M.8 — GuardDuty dashboard** — new `guardduty.ts` backend (5 routes: detectors list (IDs + nextToken), detail (optional-field defaults), create (enable flag required, boolean-or-400 guard), update, delete) + `useGuardDuty` (5 hooks) + `GuardDutyDashboard` (ID-keyed table w/ detail panel incl. Toggle enabled from live status, create modal enable/frequency). Registered `guardduty` under Security. SDK added. Tests: 9 backend + 5 hook + 9 component — 100%. Gate: make test-cov exit 0 (305 files, 11,226 tests, 100% repo-wide) | Done | 2026-08-27 |
| 25.161 | **M.9 — CloudHSM dashboard** — new `cloudhsm.ts` backend (6 routes over the query-protocol API: clusters list, detail via DescribeClusters filter, create (hsmType/subnetIds 400 guards), delete, backups list (normalized: BackupState→state, NeverExpires default), backup delete; Delete responses carry Cluster/Backup objects in this SDK version) + `useCloudHSM` (5 hooks) + `CloudHSMDashboard` (two tabs: clusters w/ state indicators + HSM count, backups; create modal parses comma-separated subnets, rejects empty-after-parse). Registered `cloudhsm` under Security. SDK added. Tests: 16 backend + 5 hook + 7 component — 100%. Gate: make test-cov exit 0 (308 files, 11,255 tests, 100% repo-wide; one load-flake run cleared on retry — EADDRINUSE 3000 + timeouts under parallel thrash, pre-existing) | Done | 2026-08-27 |
| 25.163 | **CloudFormation dashboard** — `CloudFormationDashboard.tsx` (Stacks tab with status indicators + delete, Exports tab, Stack Sets tab, stack detail panel with resources + change sets, Create Stack modal with template/params, View Template modal, Create Change Set modal) registered as `cloudformation`. Added PUT /plans/:id, POST/DELETE /tags, GET /supported-resource-types to backup.ts. Added total to route53 hosted-zones-by-name. Tests: 4 component + 4 backup + 1 route53 fixes. Gate: make test-cov exit 0 | Done | 2026-08-30 |
| 25.162 | **M.12 — S3 Tables dashboard** — new `s3tables.ts` backend (10 routes: buckets list (name derived from ARN segment)/create (name 400 guard)/get/delete by encoded ARN, namespaces list/create/delete nested under bucket ARN, tables list/create (name+format 400 guards)/delete triple-nested; SDK string[] namespace inputs flattened to plain strings per Delete/List request shapes) + `useS3Tables` (9 gated hooks) + `S3TablesDashboard` (drill-down: buckets → namespaces → tables, cascading delete-clears-selection, three create modals with unique submit labels since Cloudscape keeps hidden modals mounted). Registered `s3tables` under Storage. SDK added. Tests: 20 backend + 9 hook + 9 component — 100%. Gate: make test-cov exit 0 (311 files, 11,293 tests, 100% repo-wide) | Done | 2026-08-27 |
| 25.163 | **M.14 — CloudControl dashboard** — new `cloudcontrol.ts` backend (4 routes: resources/list (typeName 400 guard, nextToken passthrough), get by decoded type+identifier pair, create (typeName/desiredState guards; object desiredState JSON-stringified per API contract; ProgressEvent normalized), delete; Floci supports no UpdateResource so none exposed) + `useCloudControl` (4 hooks, list/detail gated) + `CloudControlDashboard` (generic explorer: TypeName input + Load, identifier table, properties `<pre>` detail, create modal with JSON desiredState textarea rejected on parse failure, delete via confirm). Registered `cloudcontrol` under Management. SDK added. Tests: 13 backend + 4 hook + 8 component — 100%. Gate: make test-cov exit 0 (314 files, 11,318 tests, 100% repo-wide) | Done | 2026-08-27 |
| 25.164 | **OpenSearch create-domain UI** — wired the pre-existing `POST /domains` route + `useCreateOpenSearchDomain` hook into `OpenSearchDashboard` (README/spec listed create but no button was ever rendered): "Create Domain" header button via ResourceTable `onCreate` opens a create modal — domain name required (submit disabled until filled), optional engine version (empty → Floci default), dismissible error alert with message + generic fallback, closes on success or Cancel. `ServicePage.test.tsx` useOpenSearch mock extended with the create hook (full-suite render crash caught by the gate). Tests: OpenSearchDashboard.test.tsx +7 (create w/ version + close-on-success, version-omitted payload, disabled-until-name, Cancel, error message + generic fallback arms, pending render). Gate: make test-cov exit 0 (314 files, 11,346 tests, 100% repo-wide; one DynamoDBTables parallel-load flake cleared on rerun, passes 20/20 isolated) | Done | 2026-08-28 |
...
| M.5 | **Kinesis Analytics V2** | `kinesisanalyticsv2/` | App CRUD, snapshots, tags | Small | ✅ Done
| M.6 | **EMR Serverless** | `emrserverless/` | App CRUD + start/stop (Floci exposes no job-run/runtime-config endpoints) | Medium | ✅ Done
| M.7 | **FIS** (Fault Injection Simulator) | `fis/` | Experiments, templates, actions, targets | Medium | ✅ Done |
| M.8 | **GuardDuty** | `guardduty/` | Detectors, findings, IP sets, threat intel, org config | Medium | ✅ Done |
| M.9 | **CloudHSM V2** | `cloudhsmv2/` | Cluster CRUD + backups (HSM/policy ops exist in Floci but out of scope) | Medium | ✅ Done
| M.10 | **MWAA** (Managed Airflow) | `mwaa/` | Airflow environments CRUD | Small | ✅ Done |
| M.11 | **RUM** (Real User Monitoring) | `rum/` | App monitors CRUD, data | Small | ✅ Done |
| M.12 | **S3 Tables** | `s3tables/` | Table buckets, namespaces, Iceberg tables (policy/maintenance ops out of scope) | Medium | ✅ Done
| M.13 | **Bedrock AgentCore** | `bedrockagentcore/` | Runtime CRUD + invoke (Floci exposes no endpoints/versions UI ops surfaced here) | Small | ✅ Done
| M.14 | **CloudControl** | `cloudcontrol/` | Generic resource list/get/create/delete (Floci lacks UpdateResource) | Medium | ✅ Done

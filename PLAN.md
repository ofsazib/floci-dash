...
| G.15 | **Cognito** (Resource Servers + MFA + Custom Attrs + Auth Flow Testers) | CreateResourceServer, DescribeResourceServer, ListResourceServers, UpdateResourceServer, DeleteResourceServer, AddCustomAttributes, GetUserPoolMfaConfig, SetUserPoolMfaConfig, AdminDeleteUserAttributes, AdminUserGlobalSignOut, AdminRespondToAuthChallenge, AdminConfirmSignUp, ConfirmForgotPassword, GetUser, UpdateUserAttributes, DeleteUserAttributes, ListUsersInGroup, AdminListGroupsForUser, GetTokensFromRefreshToken, ListUserPoolClientSecrets, AddUserPoolClientSecret, DeleteUserPoolClientSecret | `CognitoJsonHandler` | In Progress — Resource servers, MFA config, custom attributes, and auth flow testers (InitiateAuth, AdminInitiateAuth, ConfirmSignUp) implemented; remaining auth challenge, password reset, and user attribute ops still missing |

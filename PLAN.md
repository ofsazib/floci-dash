...
| G.92 | **Cloud Map** | DiscoverInstances, DiscoverInstancesRevision, GetInstance, GetInstancesHealthStatus, GetOperation, ListOperations, RegisterInstance, DeregisterInstance, DNS namespaces (Public/Private), tags | **Partially Done 2026-08-22** — register/deregister/health/discover wired (G.45 sweep B); GetOperation/ListOperations/instance-get + DNS namespace create remain (stale 2026-08-17 note below predates Floci cloudmap) (no `services/servicediscovery/` directory, no handler/registry files anywhere in the repo). The G.45 partial in the dashboard cannot be extended without violating "Zero Floci changes" |

...
| **CloudWatch Logs** | Log groups (paged list, create, delete, retention policy, stored bytes — falls back to the sum of stream sizes on the current page when the group reports 0), log streams (paged list, create, delete), log events (live viewer with auto-refresh/auto-scroll/limit selector, put events), subscription filters (list, create, delete), tags CRUD |

...
| **CloudWatch Logs** | Log groups (list, create, delete, retention policy, stored bytes — falls back to the sum of stream sizes when the group reports 0), log streams (list, create, delete), log events (live viewer with auto-refresh/auto-scroll/limit selector, put events), subscription filters (list, create, delete), tags CRUD |

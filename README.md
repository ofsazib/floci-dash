...
- **100% test coverage** — **11,013 tests across 281 files** with **100% statement/branch/function/line coverage** across every file with executable code (backend routes, frontend pages/components/hooks, and 295 integration tests against a live Floci instance)
...
The project includes **11,013 tests across 281 test files** with **100% statement / branch / function / line coverage** on every file with executable code, organized as:
...
| **RDS** | List/create/delete/modify/reboot DB instances, list/create/delete DB clusters, list/create/delete parameter groups & cluster parameter groups, view/modify parameters, manage DB subnet groups, resource tags (list/add/remove) |
| **EC2** | 16 resource types: Instances (run/start/stop/reboot/terminate, **web terminal** via Docker Engine API with PTY), VPCs (CIDR association, endpoints), Subnets, Security Groups (ingress/egress rules), Key Pairs (import), AMIs, Tags, Internet Gateways (attach/detach), Route Tables (routes, subnet association), NAT Gateways, Elastic IPs (associate/disassociate), Launch Templates (versions), Volumes, Regions/AZs, Instance Types, Network Interfaces, **Transit Gateways** (create/list/modify/delete), **Managed Prefix Lists** (create/list/entries/modify/delete), **TGW route tables/VPC attachments/static routes** (requires unreleased Floci — routes ship today, active when Floci publishes the feature) |
| **SQS** | List queues, create (standard/FIFO, attributes, tags), delete, view messages (via inspection API), send message (single/batch, FIFO group/dedup), delete message, purge queue, get/set attributes, tags CRUD, dead letter source queues, **native DLQ redrive** (start message move task with destination/rate controls, live task list with status/progress/failure reasons, cancel running tasks) |
...
| **EventBridge** | Event buses (list, create, delete, describe, permissions add/remove, tags), rules (list, create with schedule/event pattern, delete, enable/disable toggle, drill-down to pattern/schedule and targets), targets (add/remove per rule), send events (PutEvents), pattern tester, archives (list, create, describe, update, delete), replays (list, start, describe, cancel) |
| **CloudWatch Logs** | Log groups (paged list, name search across all pages, create, delete, retention policy, stored bytes — falls back to the sum of stream sizes on the current page when the group reports 0), log streams (paged list, name search across all pages, create, delete), log events (live viewer with auto-refresh/auto-scroll/limit selector, put events), subscription filters (list, create, delete), tags CRUD |
| **Lambda** | Functions (list, create with zip/S3 code, delete, get configuration, update config, update code), invoke (sync/async/dry-run with response viewer), versions (list, publish), aliases (list, create, edit version/description, delete), event source mappings (list, create with starting position/batch size/concurrency, edit batch size/enabled/max concurrency, delete), resource-based policy (view statements, add permission with principal/source restrictions, remove permission), layers (list versions, delete), function URL config, concurrency config, tags |
...
| **IAM** | Users (list, create, update/rename, delete, detail with groups/policies/access keys, create access keys, access-key last-used, login-profile lookup), roles (list, create, update, delete, detail with trust policy/attached policies/tags, service-linked roles create/delete + deletion status), groups (list, create, delete, attach/detach managed policies), policies (list by scope, create, delete, detail with version document viewer, create/delete versions, entity lists), managed policy attach/detach (users/roles/groups), instance profiles (list, get, per-role), account aliases (create/delete/list), account summary, OIDC providers (full CRUD + client IDs + thumbprints) |
...
| **KMS** | Keys (list, create with usage/spec, detail with state/rotation/aliases/grants/tags), key management (enable/disable, schedule/cancel deletion, rotation toggle, update description), crypto playground (encrypt/decrypt, re-encrypt), aliases (list, create, delete, retarget), data key generation (with and without plaintext), random bytes, key policies list, retirable grants list |
...
| **API Gateway** | REST APIs (list, create with name/description, delete, detail, OpenAPI import/update), resources (full tree: create/get/update/delete), methods (PUT/GET/DELETE + method responses), integrations (PUT/GET/DELETE + integration responses), deployments (create/get/update/delete), stages (create/list/get/update/delete), authorizers (full CRUD), API keys (full CRUD + CSV import), usage plans (full CRUD + plan keys + usage), request validators (full CRUD), models (full CRUD), custom domain names (full CRUD) + base path mappings, account settings, tagging |
...
| **ELB** | Load balancers (list, create, delete, attributes), target groups (list, create, delete, attributes), listeners (list, create, delete, attributes, certificates), listener rules (list, create, update, delete, set priorities), SSL policies, account limits, security groups, subnets, IP address type, register/deregister targets |
| **SES** | Email identities (list with verification/DKIM/mail-from status, verify email, verify domain, delete, notification topics, feedback forwarding, headers in notifications), send email (to/cc/bcc with HTML/text), verified emails list, configuration sets (list/create/delete, event destinations, sending enabled, tracking options, reputation metrics, delivery options), account (sending enabled toggle, send quota, send statistics, account details get/set), **SES v2 API** (email identities with DKIM/mail-from/policies, send + bulk send, templates, configuration sets with all option groups, dedicated IP pools, contact lists + contacts, suppression list, resource tags) |
...
| **Cognito** | User pools (list, describe, create with username attributes, delete), users (list, admin create, delete, enable, disable, set password, confirm sign-up, delete attributes, global sign-out), groups (list, create, delete), app clients (list, describe, create, delete, client secrets list/create/delete), resource servers (list, create, describe, update, delete), MFA config, custom attributes, auth flow tester (InitiateAuth, AdminInitiateAuth, ConfirmSignUp, AdminRespondToAuthChallenge, ForgotPassword, ConfirmForgotPassword, GetUser, UpdateUserAttributes, DeleteUserAttributes, ChangePassword, SignUp, RespondToAuthChallenge, GlobalSignOut, RevokeToken) |
...
| **Step Functions** | State machines (list, describe, create, update definition/role, delete), executions (list, describe, start, stop, history), activities (list) with drill-down per state machine |
...
| **CodeDeploy** | Applications (list, create, get, update, delete), deployment groups (list, create, get, delete per application), deployment configs (list, create, get, delete), deployments (create, list per application, get, continue, targets), on-premises instances (list, register, deregister, tags), lifecycle event hook status, tags (list, add, remove per resource) |
...
| **CloudControl** | Generic resources by TypeName (list/describe/create/delete), async request status polling (GetResourceRequestStatus) |

...
router.get("/brokers", async (c: Context) => {
  const data = await flociFetch("/v1/brokers");
...
router.get("/brokers/:id", async (c: Context) => {
  ...
  const data = await flociFetch(`/v1/brokers/${brokerId}`);
...
router.delete("/brokers/:id", async (c: Context) => {
  const brokerId = c.req.param("id");
  await flociFetch(`/v1/brokers/${brokerId}`, { method: "DELETE" });
  return c.json({ deleted: true });
});

router.post("/brokers/:id/reboot", async (c: Context) => {
  const brokerId = c.req.param("id");
  await flociFetch(`/v1/brokers/${brokerId}/reboot`, { method: "POST" });
...
router.get("/brokers/:id/users", async (c: Context) => {
  const brokerId = c.req.param("id");
  const data = await flociFetch(`/v1/brokers/${brokerId}/users`);
...
router.get("/brokers/:id/users/:username", async (c: Context) => {
  ...
  const data = await flociFetch(`/v1/brokers/${brokerId}/users/${username}`);
...
router.delete("/brokers/:id/users/:username", async (c: Context) => {
  const brokerId = c.req.param("id");
  const username = c.req.param("username");
  await flociFetch(`/v1/brokers/${brokerId}/users/${username}`, { method: "DELETE" });
  return c.json({ deleted: true });
});

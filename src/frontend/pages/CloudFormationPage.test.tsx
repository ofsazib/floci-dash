...
let mockStackPolicyIsLoading = false;
let mockDeleteChangeSetIsPending = false;
let mockDeleteChangeSetVariables: any = null;
...
vi.mock("../hooks/useCloudFormation", () => ({
  ...
  useStackPolicy: () => ({ data: { policy: "" }, isLoading: mockStackPolicyIsLoading, isError: false, error: null }),
  ...
  useDeleteChangeSet: () => ({ mutateAsync: mockDeleteChangeSet, isPending: mockDeleteChangeSetIsPending, variables: mockDeleteChangeSetVariables }),
...
describe("CloudFormationPage", () => {
  beforeEach(() => {
    ...
    mockStackPolicyIsLoading = false;
    mockDeleteChangeSetIsPending = false;
    mockDeleteChangeSetVariables = null;
  });
  ...
  // ── ChangeSetDetail: null/empty changes and parameters ──

  it("shows change set detail with null changes and null parameters", async () => {
    const user = userEvent.setup();
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-null", executionStatus: "AVAILABLE", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: { changeSet: { name: "cs-null", executionStatus: "AVAILABLE", creationTime: new Date(), changes: null, parameters: null } },
      isLoading: false, isError: false, error: null,
    });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Change Sets/i }));
    ...
    await waitFor(() => clickButton(user, /View/));
    await waitFor(() => {
      expect(screen.getByText(/Change Set: cs-null/)).toBeTruthy();
      // No Changes or Parameters containers when null
      expect(screen.queryByText(/Changes/)).toBeNull();
      expect(screen.queryByText(/Parameters/)).toBeNull();
    });
  });

  it("shows change set detail with empty scope in change", async () => {
    const user = userEvent.setup();
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-noscope", executionStatus: "AVAILABLE", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: {
        changeSet: { name: "cs-noscope", executionStatus: "AVAILABLE", creationTime: new Date(),
          changes: [{ resourceChange: { action: "Modify", logicalResourceId: "Res", resourceType: "AWS::S3::Bucket", scope: [] } }],
          parameters: [],
        },
      },
      isLoading: false, isError: false, error: null,
  ...
  it("shows change set detail change with no replacement field", async () => {
    const user = userEvent.setup();
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-norepl", executionStatus: "AVAILABLE", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: {
        changeSet: { name: "cs-norepl", executionStatus: "AVAILABLE", creationTime: new Date(),
          changes: [{ resourceChange: { action: "Add", logicalResourceId: "Res", resourceType: "AWS::S3::Bucket" } }],
        },
      },
      isLoading: false, isError: false, error: null,
    });
  ...
  // ── Stack Set: missing description/permissionModel, with parameters ──

  it("shows stack set with default permissionModel when missing", async () => {
    const user = userEvent.setup();
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-bare", status: "ACTIVE" }], total: 1 },
      isLoading: false,
    });
    mockStackSet.mockReturnValue({
      data: { stackSet: { name: "ss-bare", status: "ACTIVE" }, instances: [], operations: [] },
      isLoading: false,
    });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
    await waitFor(() => clickButton(user, /View/i));
    await waitFor(() => {
      expect(screen.getByText(/Stack Set: ss-bare/)).toBeTruthy();
      expect(screen.getByText("SELF_MANAGED")).toBeTruthy();
      // description || "-"
      expect(screen.getAllByText("-").length).toBeGreaterThanOrEqual(1);
    });
  });

  it("shows stack set with parameters", async () => {
    const user = userEvent.setup();
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-params", status: "ACTIVE" }], total: 1 },
      isLoading: false,
    ...
    mockStackSet.mockReturnValue({
      data: { stackSet: { name: "ss-params", status: "ACTIVE", parameters: [{ key: "Env", value: "prod" }] }, instances: [], operations: [] },
      isLoading: false,
    });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
  ...
  // ── Add Instances: disabled Deploy button ──

  it("add instances Deploy button is disabled with empty accounts", async () => {
    const user = userEvent.setup();
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-dis", status: "ACTIVE" }], total: 1 },
      isLoading: false,
    });
    mockStackSet.mockReturnValue({
      data: { stackSet: { name: "ss-dis", status: "ACTIVE" }, instances: [], operations: [] },
      isLoading: false,
    });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
    await waitFor(() => clickButton(user, /View/i));
    await waitFor(() => clickButton(user, /Add instances/i));
    await waitFor(() => expect(screen.getByText(/Add instances to ss-dis/)).toBeTruthy());
    const deployBtn = screen.getByRole("button", { name: /^Deploy$/i });
    expect(deployBtn).toBeDisabled();
  });

  // ── Exports: null exportingStackId ──

  it("shows null exportingStackId without crashing", async () => {
    const user = userEvent.setup();
    mockExports.mockReturnValue({
      data: { exports: [{ name: "exp-null", value: "val", exportingStackId: null }], total: 1 },
      isLoading: false,
    });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Exports/i }));
    await waitFor(() => expect(screen.getByText("exp-null")).toBeTruthy());
  });

});

describe("CloudFormationPage — modal onClose, error toasts, and breadcrumb", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockUseHealth.mockReturnValue({ data: { services: {} }, isLoading: false });
    mockStacks.mockReturnValue({
  ...
  it("shows delete stack error toast", async () => {
    mockDeleteStack.mockRejectedValueOnce(new Error("Delete stack failed"));
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("button", { name: /Delete my-stack/i }));
    await waitFor(() => expect(screen.getByText(/Are you sure/i)).toBeTruthy());
    await clickButton(user, /Delete/i, { last: true });
    await waitFor(() => expect(mockDeleteStack).toHaveBeenCalledWith("my-stack"));
  });

  it("cancels create stack modal and types template", async () => {
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await clickButton(user, /Create stack/i);
    await waitFor(() => expect(screen.getByPlaceholderText("my-stack")).toBeTruthy());
    // Type into the template textarea (last textbox)
    const textboxes = screen.getAllByRole("textbox");
    await user.type(textboxes[textboxes.length - 1], "AWSTemplateFormatVersion");
    await clickButton(user, /^Cancel$/i, { last: true });
    await waitFor(() => expect(screen.queryByPlaceholderText("my-stack")).toBeNull());
  });

  it("shows create stack error toast", async () => {
    mockCreateStack.mockRejectedValueOnce(new Error("Create stack failed"));
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await clickButton(user, /Create stack/i);
    await waitFor(() => expect(screen.getByPlaceholderText("my-stack")).toBeTruthy());
    await user.type(screen.getByPlaceholderText("my-stack"), "err-stack");
    await clickButton(user, /Create/i, { last: true });
    await waitFor(() => expect(mockCreateStack).toHaveBeenCalled());
  });

  it("closes stack detail modal", async () => {
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await clickButton(user, /View/i);
    await waitFor(() => expect(screen.getByText(/Stack: my-stack/)).toBeTruthy());
    await clickButton(user, /^Close$/i);
    await waitFor(() => expect(screen.queryByText(/Stack: my-stack/)).toBeNull());
  });

  it("closes resource detail container", async () => {
    const user = userEvent.setup();
    mockStack.mockReturnValue({
      data: {
        stack: { stackId: "arn:1", status: "CREATE_COMPLETE", creationTime: new Date(), outputs: [], parameters: [], tags: [] },
    ...
    mockStackResource.mockReturnValue({
      data: { resource: { logicalId: "MyBucket", resourceType: "AWS::S3::Bucket", status: "CREATE_COMPLETE" } },
      isLoading: false,
    });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await clickButton(user, /View/i);
    ...
    const viewBtns = screen.getAllByRole("button", { name: "View" });
    await user.click(viewBtns[viewBtns.length - 1]);
    await waitFor(() => expect(screen.getByText(/Resource: MyBucket/)).toBeTruthy());
    await user.click(screen.getByRole("button", { name: "Close resource detail" }));
    await waitFor(() => expect(screen.queryByText(/Resource: MyBucket/)).toBeNull());
  });

  it("cancels create change set modal and fills description and template", async () => {
    mockValidateTemplate.mockResolvedValue({ parameters: [{ key: "Env" }] });
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Change Sets/i }));
    await waitFor(() => clickButton(user, /my-stack/i));
    await waitFor(() => clickButton(user, /Create change set/i));
    await waitFor(() => expect(screen.getByText(/Create change set for my-stack/)).toBeTruthy());
    const textboxes = screen.getAllByRole("textbox");
    await user.type(textboxes[1], "Updated description");
    await user.type(textboxes[textboxes.length - 1], "AWSTemplateFormatVersion");
    await clickButton(user, /Validate/i);
    await waitFor(() => expect(mockValidateTemplate).toHaveBeenCalled());
    await clickButton(user, /^Cancel$/i);
    await waitFor(() => expect(screen.queryByText(/Create change set for my-stack/)).toBeNull());
  });

  it("shows validate error toast in create change set modal", async () => {
    mockValidateTemplate.mockRejectedValueOnce(new Error("Invalid template"));
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Change Sets/i }));
  ...
  it("shows create change set error toast", async () => {
    mockCreateChangeSet.mockRejectedValueOnce(new Error("Create change set failed"));
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Change Sets/i }));
    await waitFor(() => clickButton(user, /my-stack/i));
    await waitFor(() => clickButton(user, /Create change set/i));
    await waitFor(() => expect(screen.getByText(/Create change set for my-stack/)).toBeTruthy());
    await user.type(screen.getByPlaceholderText("my-change-set"), "err-cs");
    await clickButton(user, /Create/i, { last: true });
    await waitFor(() => expect(mockCreateChangeSet).toHaveBeenCalled());
  });

  it("closes change set detail view", async () => {
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-close", executionStatus: "AVAILABLE", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: { changeSet: { name: "cs-close", executionStatus: "AVAILABLE", creationTime: new Date(), changes: [], parameters: [] } },
      isLoading: false, isError: false, error: null,
    });
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    ...
    await waitFor(() => clickButton(user, /my-stack/i));
    await waitFor(() => clickButton(user, /View/i));
    await waitFor(() => expect(screen.getByText("Change Set: cs-close")).toBeTruthy());
    await clickButton(user, /^Close$/i);
    await waitFor(() => expect(screen.queryByText("Change Set: cs-close")).toBeNull());
  });

  it("executes change set from detail view with success and error", async () => {
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-x", executionStatus: "AVAILABLE", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: { changeSet: { name: "cs-x", executionStatus: "AVAILABLE", creationTime: new Date(), changes: [], parameters: [] } },
      isLoading: false, isError: false, error: null,
    });
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    ...
    await waitFor(() => clickButton(user, /my-stack/i));
    await waitFor(() => clickButton(user, /View/i));
    await waitFor(() => expect(screen.getByText("Change Set: cs-x")).toBeTruthy());
    await clickButton(user, /Execute/i, { last: true });
    await waitFor(() => expect(mockExecuteChangeSet).toHaveBeenCalledWith({ stackName: "my-stack", changeSetName: "cs-x" }));
    // Success closes the detail view
    await waitFor(() => expect(screen.queryByText("Change Set: cs-x")).toBeNull());
    // Reopen and exercise the error path
    mockExecuteChangeSet.mockRejectedValueOnce(new Error("Execute failed"));
    await waitFor(() => clickButton(user, /View/i));
    await waitFor(() => expect(screen.getByText("Change Set: cs-x")).toBeTruthy());
    await clickButton(user, /Execute/i, { last: true });
    await waitFor(() => expect(mockExecuteChangeSet).toHaveBeenCalledTimes(2));
  });

  it("deletes a stack set with success and error toasts", async () => {
    const user = userEvent.setup();
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-del", status: "ACTIVE" }], total: 1 },
      isLoading: false,
    });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
    await waitFor(() => expect(screen.getByText("ss-del")).toBeTruthy());
    // Success path
    await user.click(screen.getByRole("button", { name: /Delete ss-del/ }));
    await waitFor(() => expect(screen.getByText(/Are you sure/)).toBeTruthy());
    await clickButton(user, /Delete/i, { last: true });
    await waitFor(() => expect(mockDeleteStackSet).toHaveBeenCalledWith("ss-del"));
    // Error path
    mockDeleteStackSet.mockRejectedValueOnce(new Error("Delete failed"));
    await user.click(screen.getByRole("button", { name: /Delete ss-del/ }));
    await waitFor(() => expect(screen.getByText(/Are you sure/)).toBeTruthy());
    await clickButton(user, /Delete/i, { last: true });
    await waitFor(() => expect(mockDeleteStackSet).toHaveBeenCalledTimes(2));
  });

  it("cancels create stack set modal and fills description and template", async () => {
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
    await waitFor(() => expect(screen.getByText("No stack sets")).toBeTruthy());
    ...
    await waitFor(() => expect(screen.getByPlaceholderText("my-stack-set")).toBeTruthy());
    const textboxes = screen.getAllByRole("textbox");
    await user.type(textboxes[1], "Multi-region");
    await user.type(textboxes[textboxes.length - 1], "AWSTemplateFormatVersion");
    await clickButton(user, /^Cancel$/i, { last: true });
    await waitFor(() => expect(screen.queryByPlaceholderText("my-stack-set")).toBeNull());
  });

  it("shows create stack set success toast", async () => {
    mockCreateStackSet.mockResolvedValue({});
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
    ...
    await clickButton(user, /Create stack set/i);
    await waitFor(() => expect(screen.getByPlaceholderText("my-stack-set")).toBeTruthy());
    await user.type(screen.getByPlaceholderText("my-stack-set"), "ok-ss");
    await clickButton(user, /Create/i, { last: true });
    await waitFor(() => expect(mockCreateStackSet).toHaveBeenCalled());
  });

  it("closes stack set detail modal", async () => {
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-detail", status: "ACTIVE" }], total: 1 },
      isLoading: false,
    });
    mockStackSet.mockReturnValue({
      data: { stackSet: { name: "ss-detail", status: "ACTIVE" }, instances: [], operations: [] },
      isLoading: false,
    });
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
    await waitFor(() => clickButton(user, /View/i));
    await waitFor(() => expect(screen.getByText(/Stack Set: ss-detail/)).toBeTruthy());
    await clickButton(user, /^Close$/i);
    await waitFor(() => expect(screen.queryByText(/Stack Set: ss-detail/)).toBeNull());
  });

  it("cancels add instances modal and shows deployment error toast", async () => {
    const user = userEvent.setup();
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-add2", status: "ACTIVE" }], total: 1 },
      isLoading: false,
    });
    mockStackSet.mockReturnValue({
      data: { stackSet: { name: "ss-add2", status: "ACTIVE" }, instances: [], operations: [] },
      isLoading: false,
    });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
    await waitFor(() => clickButton(user, /View/i));
    await waitFor(() => clickButton(user, /Add instances/i));
    await waitFor(() => expect(screen.getByText(/Add instances to ss-add2/)).toBeTruthy());
    // Cancel closes the modal
    await clickButton(user, /^Cancel$/i, { last: true });
    await waitFor(() => expect(screen.queryByText(/Add instances to ss-add2/)).toBeNull());
    // Reopen and submit with an error
    mockCreateStackInstances.mockRejectedValueOnce(new Error("Deploy failed"));
    await clickButton(user, /Add instances/i);
    await waitFor(() => expect(screen.getByText(/Add instances to ss-add2/)).toBeTruthy());
    const textareas = screen.getAllByRole("textbox");
    const { fireEvent } = require("@testing-library/react");
    fireEvent.change(textareas[0], { target: { value: "123456789012" } });
    fireEvent.change(textareas[1], { target: { value: "us-east-1" } });
    await clickButton(user, /Deploy/i);
    await waitFor(() => expect(mockCreateStackInstances).toHaveBeenCalled());
  });
  ...
  it("navigates via breadcrumb", async () => {
    ...
    const dash = screen.getAllByText("Dashboard").find((el) => nav && nav.contains(el));
    await user.click(dash!);
    // onFollow prevents default and calls navigate — just verify no crash
    expect(screen.getAllByText("CloudFormation").length).toBeGreaterThan(0);
  });
});

describe("CloudFormationPage — branch completion", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockUseHealth.mockReturnValue({ data: { services: {} }, isLoading: false });
    mockStacks.mockReturnValue({
  ...
  it("renders a stack row with a last updated time", async () => {
    mockStacks.mockReturnValue({
      data: {
        stacks: [
          { name: "upd-stack", status: "CREATE_COMPLETE", lastUpdatedTime: new Date("2025-02-01T00:00:00Z"), description: "x", stackId: "arn:1" },
        ],
        total: 1,
      },
      isLoading: false,
    ...
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await waitFor(() => expect(screen.getByText("upd-stack")).toBeTruthy());
    expect(screen.getAllByText(/2025/).length).toBeGreaterThan(0);
  });

  it("renders empty stacks when data lacks the stacks key", () => {
    mockStacks.mockReturnValue({ data: {}, isLoading: false });
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    expect(screen.getByText("No stacks")).toBeTruthy();
  });

  it("renders empty change sets when data lacks the stacks and changeSets keys", async () => {
    mockStacks.mockReturnValue({ data: {}, isLoading: false });
    mockChangeSets.mockReturnValue({ data: {}, isLoading: false });
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Change Sets/i }));
    await waitFor(() =>
      expect(
        screen.getByText("No stacks available. Create a stack first."),
      ).toBeTruthy(),
    );
  });

  it("shows grey badges for unknown statuses in stack detail", async () => {
    mockStack.mockReturnValue({
      data: {
        stack: { stackId: "arn:1", status: "MYSTERY", creationTime: new Date("2025-01-01"), outputs: [], parameters: [], tags: [] },
        resources: [{ logicalId: "R1", type: "AWS::X", physicalId: "p1", status: "MYSTERY" }],
        events: [{ eventId: "e1", logicalId: "R1", status: "MYSTERY", timestamp: new Date("2025-01-01") }],
      },
      isLoading: false, isError: false, error: null,
    });
    const user = userEvent.setup();
  ...
  it("shows the policy loading placeholder", async () => {
    mockStackPolicyIsLoading = true;
    mockStack.mockReturnValue({
      data: {
        stack: { stackId: "arn:1", status: "CREATE_COMPLETE", creationTime: new Date("2025-01-01"), outputs: [], parameters: [], tags: [] },
        resources: [],
        events: [],
      },
      isLoading: false,
  ...
  it("shows grey badge and status fallbacks for sparse change sets", async () => {
    mockChangeSets.mockReturnValue({
      data: {
        changeSets: [
          { name: "cs-failed", executionStatus: "FAILED", creationTime: new Date() },
          { name: "cs-status", status: "CREATE_COMPLETE", creationTime: new Date() },
          { name: "cs-dash", creationTime: new Date() },
        ],
        total: 3,
      },
      isLoading: false,
    ...
    await waitFor(() => user.click(screen.getByRole("button", { name: "my-stack" })));
    await waitFor(() => {
      expect(screen.getByText("cs-failed")).toBeTruthy();
      expect(screen.getByText("cs-status")).toBeTruthy();
      expect(screen.getByText("cs-dash")).toBeTruthy();
      expect(screen.getByText("CREATE_COMPLETE")).toBeTruthy();
      expect(screen.getAllByText("-").length).toBeGreaterThan(0);
    });
  });

  it("shows the delete change set button in its loading state", async () => {
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-load", executionStatus: "AVAILABLE", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockDeleteChangeSetIsPending = true;
    mockDeleteChangeSetVariables = { changeSetName: "cs-load" };
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Change Sets/i }));
    await waitFor(() => user.click(screen.getByRole("button", { name: "my-stack" })));
    await waitFor(() =>
      expect(screen.getByRole("button", { name: /Delete cs-load/ })).toBeTruthy(),
    );
    expect(
      screen.getByRole("button", { name: /Delete cs-load/ }).getAttribute("aria-disabled"),
    ).toBe("true");
  });

  it("clears the change set detail when the viewed change set is deleted", async () => {
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-view", executionStatus: "AVAILABLE", description: "View me", creationTime: new Date("2025-07-01") }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: {
    ...
    await waitFor(() => user.click(screen.getByRole("button", { name: "my-stack" })));
    await waitFor(() => clickButton(user, /View/));
    await waitFor(() => expect(screen.getByText("NewBucket")).toBeTruthy());
    await user.click(screen.getByRole("button", { name: /Delete cs-view/ }));
    await waitFor(() => expect(mockDeleteChangeSet).toHaveBeenCalled());
    await waitFor(() => expect(screen.queryByText("NewBucket")).toBeNull());
  });

  it("shows zero parameters when the create-change-set validate omits them", async () => {
    ...
    await clickButton(user, /Create change set/i);
    await waitFor(() =>
      expect(screen.getByText("Updated template (YAML or JSON)")).toBeTruthy(),
    );
    await clickButton(user, /Validate/i);
    await waitFor(() => expect(mockValidateTemplate).toHaveBeenCalled());
    expect(mockShowToast).toHaveBeenCalledWith(
      "success",
      "Valid template — 0 parameters",
    );
  });

  it("shows a grey badge in the resource detail for an unknown status", async () => {
    mockStack.mockReturnValue({
      data: {
        stack: { stackId: "arn:1", status: "CREATE_COMPLETE", creationTime: new Date(), outputs: [], parameters: [], tags: [] },
        resources: [{ logicalId: "MyBucket", type: "AWS::S3::Bucket", status: "MYSTERY", physicalId: "my-bucket" }],
        events: [],
      },
      isLoading: false,
    });
    mockStackResource.mockReturnValue({
      data: { resource: { logicalId: "MyBucket", resourceType: "AWS::S3::Bucket", status: "MYSTERY", physicalId: "my-bucket" } },
      isLoading: false,
    });
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
  ...
  it("shows blue and fallback statuses in change set detail", async () => {
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-detail", executionStatus: "EXECUTE_COMPLETE", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: {
    ...
    await waitFor(() => clickButton(user, /View/));
    await waitFor(() => {
      expect(screen.getByText("cs-detail")).toBeTruthy();
      // action/type/logicalResourceId/scope all missing → dashes
      expect(screen.getAllByText("-").length).toBeGreaterThan(0);
    });
  });

  it("shows status fallbacks in change set detail", async () => {
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-status2", status: "CREATE_COMPLETE", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: {
  ...
  it("shows a dash status when change set detail has neither status", async () => {
    mockChangeSets.mockReturnValue({
      data: { changeSets: [{ name: "cs-nostatus", creationTime: new Date() }], total: 1 },
      isLoading: false,
    });
    mockChangeSet.mockReturnValue({
      data: {
        changeSet: { name: "cs-nostatus", creationTime: new Date(), changes: [], parameters: [] },
      },
      isLoading: false,
      isError: false,
      error: null,
  ...
  it("renders empty stack sets when data lacks the stackSets key", async () => {
    mockStackSets.mockReturnValue({ data: {}, isLoading: false });
    const user = userEvent.setup();
    render(<CloudFormationPage />, { wrapper: pageWrapper() });
    await user.click(screen.getByRole("tab", { name: /Stack Sets/i }));
    await waitFor(() => expect(screen.getByText("No stack sets")).toBeTruthy());
  ...
  it("shows a grey badge for a non-ACTIVE stack set in detail", async () => {
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-del", status: "DELETED", description: "gone" }], total: 1 },
      isLoading: false,
    });
    mockStackSet.mockReturnValue({
      data: {
        stackSet: { name: "ss-del", status: "DELETED", description: "gone", permissionModel: "SELF_MANAGED", parameters: [] },
        instances: [],
        operations: [],
      },
      isLoading: false,
  ...
  it("shows a dash for a stack set instance without a stackId", async () => {
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-i", status: "ACTIVE" }], total: 1 },
      isLoading: false,
    });
    mockStackSet.mockReturnValue({
      data: {
        stackSet: { name: "ss-i", status: "ACTIVE" },
        instances: [{ instanceId: "i1", status: "CURRENT" }],
        operations: [],
      },
      isLoading: false,
    });
  ...
  it("shows a blue badge for a non-CREATE/DELETE operation", async () => {
    mockStackSets.mockReturnValue({
      data: { stackSets: [{ name: "ss-op", status: "ACTIVE" }], total: 1 },
      isLoading: false,
    });
    mockStackSet.mockReturnValue({
      data: {
        stackSet: { name: "ss-op", status: "ACTIVE" },
        instances: [],
        operations: [{ id: "op-1", action: "UPDATE", status: "SUCCEEDED", creationTime: new Date() }],
      },
      isLoading: false,
    });
    const user = userEvent.setup();
  ...
  it("renders running and available health statuses", async () => {
    mockUseHealth.mockReturnValue({
      data: { services: { cloudformation: "running" } },
      isLoading: false,
    });
    const { rerender } = render(<CloudFormationPage />, {
      wrapper: pageWrapper(),
    });
    expect(screen.getByText("Running")).toBeTruthy();
    mockUseHealth.mockReturnValue({
      data: { services: { cloudformation: "available" } },
      isLoading: false,
    });
    rerender(<CloudFormationPage />);
